...
```{r}
secchi.spatial <- st_as_sf(
  secchi.summary, coords = c("long", "lat"), crs = 4326
  )
...
LAGOScombined <- LAGOScombined %>%
  mutate(decade = year(floor_date(sampledate, years (10))))
...
secchi.decade.spatial <- st_as_sf(
  secchi.summary.decade, coords = c("long", "lat"), crs = 4326
  )
